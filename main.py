...
i1=ImageTk.PhotoImage(Image.open("assets/logo.png"))
l=Label(image=i1)
l.pack(side=TOP)
def forget():
    global k,b,tp
...
def find(e):
    ...
    cursor.execute("select * from report_card where %s in (adm_no,roll_no,name,class_,sec,subject,pt1,pt2,pt3,pb1,cb,total,per)", (e.get(),))
...
photo=PhotoImage(file=r"assets/sps.png")
...
def edit():
    ...
    def edit1():
        try:
            cursor.execute("select * from report_card where adm_no=%s", (ad.get(),))
...
def report():
    ...
    def report1():
        ...
        t.delete(1.0,'end')
        try:
            im=PhotoImage(file="assets/logo.png")
